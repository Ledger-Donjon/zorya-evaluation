...
binary_path = "/xxx/pcode-generator/tests/tinygo_too-large-channel-creation/tinygo_too-large-channel-creation"
main_address = 0x2036e6

MAX_ALLOWED_CHANNEL_SIZE = 2**64 - 1
...
def detect_large_channel_size(ircfg_map):
    """
    Analyze the IRCFG to detect cases of channel creation with an overly large size.
    """
    large_channel_violations = []
    ...
    for addr, ircfg in ircfg_map.items():
        for lbl, irblock in ircfg.blocks.items():
            for assignblk in irblock:
                ...
                # Look for make(chan) arguments with a large size
                for src in read_elements:
                    if isinstance(src, ExprInt) and src.arg > MAX_ALLOWED_CHANNEL_SIZE:
                        large_channel_violations.append((lbl, assignblk))
                        print(f"Large channel size detected at block {lbl}, instruction: {assignblk}")

    return large_channel_violations
...
def main():
    ...
    # Detect large channel size
    large_channel_violations = detect_large_channel_size(ircfg_map)

    if not large_channel_violations:
        print("No large channel size issues detected.")
    else:
        print(f"Detected {len(large_channel_violations)} potential large channel size issues.")
        for lbl, assignblk in large_channel_violations:
